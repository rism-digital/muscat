  ...
  def active_admin_user_wf( context, item )   
    context.panel (I18n.t :filter_wf) do
      context.attributes_table_for item  do
        ...
        context.row (I18n.t :created_at) { |r| I18n.localize(r.created_at.localtime, :format => '%A %e %B %Y - %H:%M') } if (r.created_at)
        context.row (I18n.t :updated_at) { |r| I18n.localize(r.updated_at.localtime, :format => '%A %e %B %Y - %H:%M') } if (r.updated_at)
      end
    end
  end
