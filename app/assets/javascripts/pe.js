	...
	$(".pe_group_contents", form).each(function (index, elem) {
		...
		$(elem).children().each(function () {
			...
			$('dt', this).each(function() {
				...
				//marc_tag = {"000": {"subfields":[], "ind1":"", "ind2":""} };
				var subfields = [];
				var tag;
				
				$('.serialize_marc', this).each(function() {
				...
				if (json_marc["fields"].length == 0) {
					...
					for (var i = 0; i < json_marc["fields"].length; i++) { 
						...
						for (var marc_num in marc_tag) break;
												
						//console.log(marc_num);
						//console.log(field_num);
						
						if (marc_num < field_num) {
