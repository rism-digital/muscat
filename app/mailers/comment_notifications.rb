class CommentNotifications < ApplicationMailer

  def parse_comment(comment)
    matches = comment.scan(/@[^{ \n@}]*/)

    [] if matches.empty?
    
    normalized_sql = "LOWER(TRIM(REGEXP_REPLACE(REPLACE(name,'_',' '),'[^[:alnum:]_[:space:]]','')))"

    return matches.map do |name|
      # Stip all punctuation except for the _
      sanitized_name = name.gsub("_", " ").gsub(/[^\w\p{L}\s]/, "").strip.downcase
      # Match on the sanitized string, this works as it was always
      # if it is nil, then try to match by sanitizing the db column
      # We want both so we can keep the current expected functionality as is
      User.where("lower(name) = ?", sanitized_name).pick(:id) ||
        User.where("#{normalized_sql} = ?", sanitized_name).pick(:id)
    end.compact

  end

  def new_comment(comment)
    
    all_comments = ActiveAdmin::Comment.where(resource_type: comment.resource_type, resource_id: comment.resource_id)
    @comment = comment
    # Try to get the owner of the resource
    @resource = Kernel.const_get(comment.resource_type).find(comment.resource_id)
    @commenter = User.find(comment.author_id)
    
    model_for_path = comment.resource_type.underscore.downcase
    link_function = "admin_#{model_for_path}_url"
    @resource_path = send(link_function, @resource)
    
    users = all_comments.map {|c| c.author_id}
    users << @resource.wf_owner
    all_comments.each {|c| users += parse_comment(c.body)}
    users = users.compact.sort.uniq # Users can be duplicated
    users -= [comment.author_id] # Don't send the comment to myself!

    addresses = users.compact.each.map do |u|
      user = User.find_by(id: u) # nil if missing

      # For users 0 (no name) and 1 (admin), send a mail not
      # to the email supplied in the user configuration
      # but to a fixed mail specified in ADMIN_MAIL_SINK_USER
      # if this variable is defined. If not defined, just skip
      # users 0 and 1
      if [0, 1].include?(u)
        # No sink user, no email sent
        next unless defined?(ADMIN_MAIL_SINK_USER)
        sink = User.find_by(id: ADMIN_MAIL_SINK_USER)
        # Oops user ID does not exist
        next unless sink
        user = sink
      end

      # Make sure we don't implode but just skip this user
      # It should never happen...
      next if user.nil?

      # If the user has a notification email, use that
      # This has the precedence to any other method
      if user.notification_email && !user.notification_email.empty?
        user.notification_email
      else
        # If the user is disabled AND it has no
        # notification email, try to get the workgrups default one(s)
        if user.disabled?
          user.get_workgroups_emails
        else
          # The user is not disabled and it has no default
          # email sink, use the normal email
          user.email if user.email
        end
      end
    end.compact
     
    #addresses << RISM::COMMENT_EMAIL if (RISM::COMMENT_EMAIL && RISM::COMMENT_EMAIL.is_a?(String))

    to = RISM::DEFAULT_NOREPLY_EMAIL
    to = RISM::COMMENT_EMAIL if (RISM::COMMENT_EMAIL && RISM::COMMENT_EMAIL.is_a?(String))

    return if addresses.empty?

    mail(to: to,
        name: "Muscat",
        bcc: addresses,
        subject: "[Muscat] Comment on #{comment.resource_type} #{comment.resource_id}")
 
  end

end
