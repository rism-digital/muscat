ActiveAdmin.register CanonicImitation do

  menu :parent => "indexes_menu", :label => proc {I18n.t(:menu_canonic_imitation)}
  ...
  controller do
    ...
    def index
      @results, @hits = CanonicImitation.search_as_ransack(params)

      index! do |format|
        @canonic_imitations = @results
        format.html
      end
    end
    ...
    def show
      begin
        @canonic_imitation = CanonicImitation.find(params[:id])
      rescue ActiveRecord::RecordNotFound
        redirect_to admin_root_path, :flash => { :error => "#{I18n.t(:error_not_found)} (Canonic Imitation #{params[:id]})" }
        return
      end
      @prev_item, @next_item, @prev_page, @next_page = CanonicImitation.near_items_as_ransack(params, @canonic_imitation)

      # @jobs = @canonic_imitation.delayed_jobs
    end
  ...
  member_action :reindex, method: :get do
    job = Delayed::Job.enqueue(ReindexItemsJob.new(CanonicImitation.find(params[:id]), "refering_techniques"))
  ...
  index :download_links => false do
    ...
    column (I18n.t :display_interval) do |canonic_imitation|
      "#{canonic_imitation.interval} #{canonic_imitation.interval_direction}"
    end
    column (I18n.t :display_temporal_offset) do |canonic_imitation|
      offset = "#{canonic_imitation.temporal_offset} #{canonic_imitation.offset_units}"
      if canonic_imitation.mensurations.blank?
        offset
      else
        "#{offset} in #{canonic_imitation.mensurations}"
      end
    end
  ...
  show do
    ...
    active_admin_embedded_source_list( self, canonic_imitation, !is_selection_mode? )
    active_admin_user_wf( self, canonic_imitation )
    active_admin_navigation_bar( self )
    active_admin_comments if !is_selection_mode?
  end

  sidebar :actions, :only => :show do
    render :partial => "activeadmin/section_sidebar_show", :locals => { :item => canonic_imitation }
  end
  ...
  sidebar :actions, :only => [:edit, :new, :update] do
    render :partial => "activeadmin/section_sidebar_edit", :locals => { :item => canonic_imitation }
  end
