...
This will create the directory layout as above. It will also create a `solr` user. For the complete reference see [https://solr.apache.org/guide/8\_8/taking-solr-to-production.html](https://solr.apache.org/guide/8_8/taking-solr-to-production.html) Now copy over the Muscat core:

```
cp -R $MUSCAT_HOME/solr-configuration/muscat /data/solr/data
chown -R solr:solr /data/solr/dat/muscat
```
...
Alternatively, you can take advantatge of the Apache itk module, that simplifies not only having more than one Muscat in a single server using virtual hosts, but also the whole file permissions and ownership altogether. This Apache module ([http://mpm-itk.sesse.net/](http://mpm-itk.sesse.net/)) makes the Apache web server to run as a plain user, so it is the owner of the files, and it is no longer necessary file ownership via chown or users via sudo.  Apache-itk is an official Debian and Ubuntu package, so:
