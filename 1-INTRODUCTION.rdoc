...
This way all the basic information related to a Source is always available on the database. Marc21 records and these classes are automatically kept in sync, using ids to relate the various entities.

The web interface is a standard Rails 4 application following the MVC paradigm.
...
How the Marc21 data is managed, and how the interaction between Marc records and database-stored elements is configured in the tag configuration, in <tt>config/marc/tag_config*.yml</tt>, one for each model that supports Marc. This also defines how records should be handled by the system (for example, if a tag is repeated).
