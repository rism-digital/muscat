...
class Marc21Import
  ...
  def import
    buffer = ""
    line_number = 0
      marcblock = buildMarcBlock
      marcblock.each do |record|
        rec=Nokogiri::XML(record.to_s)
        xslt  = Nokogiri::XSLT(File.read('/home/dev/projects/muscat/housekeeping/import/marcxml2marctxt_record.xsl'))
  ...
  def create_record(buffer, line_number=0)
    ...
    if @total_records >= @from
      ...
      if marc.is_valid?(false)
        ...
        manuscript = Object.const_get(@model).find_by_id( marc.get_marc_source_id )
        if !manuscript
          manuscript = Object.const_get(@model).new(:wf_owner => 1, :wf_stage => "published", :wf_audit => "approved")
        end
...
if ARGV.length >= 2
  ...
  if File.exists?(source_file)
    import = Marc21Import.new(source_file, model, from.to_i)
    import.import
  else
