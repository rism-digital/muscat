...
The source code will be available from the google project page http://code.google.com/p/muscat-project/

The documentation for version 1.0 is available at http://docs.rism-ch.org/muscat/1.0/
...
This way all the basic information related to a Manuscript is always available on the database. Marc21 records and these classes are automatically kept in sync, using UUIDS to relate the various entities.
...
Full-text search on the data is handled using the Ferret full-text index. Every time a Manuscript is saved, relevant data is extracted and sent to the index. How the various fields and Marc tags are indexed is controlled by the configuration file in <tt>config/ferret/manuscript_index.1.0.yml</tt>.
...
Lastly, an implementation of METS (http://www.loc.gov/standards/mets) permits the linking of Digital Objects (Do* classes) to each manuscript. For the moment, only linking and showing images (using DoFile and DoImage) is implemented, but the system is designed to accept othe media types.
