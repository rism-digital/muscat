# The statistic class is a helper class to create and reuse statistical data from objects
# Main core is an array of ordered objects with key-value pairs in a hash. 
# This array will be generated by the ItemFactory.
...
class Statistic
  ...
  # This class takes an array of objects and creates the item objects using the builder function
  # row_builder function of the object should return an ordered hash.
  class ItemFactory
    def self.build(objects, row_builder, *att)
      objects.map { |obj|
        row = obj.send(row_builder, *att)
        raise "Row builder function not returning a hash" unless row.is_a?(Hash)
        Item.new(obj, row)
      }
    end
  end

  # Member of the statistic class
  # Attributes are
  # - object = Object with 
  # - row = containing a hash
  class Item
