...
The simplest way to obtain Marc records in such a format is to use the <tt>marcxml2marctxt</tt> XSL stylesheet available in the <tt>./public/xsl/</tt> directory. A sample MarcXML file is provided (+./housekeeping/import/sample.xml+). Records can be converted with a XSLT 1.0 processor, for example with the following command:

 xsltproc marcxml2marctxt.xsl input.xml > ouput.txt
...
related to any of them. This field is not updated during import, and a separate script needs to be run afterwards:
	...
	./script/runner housekeeping/correct/27_update_mscount.rb
