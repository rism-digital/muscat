...
RSpec.describe Admin::SourcesController, :type => :feature, :js => :true do 
  ...
  it "cannot create source from foreign library" do
    set_field("852$x", foreign_institution.id)
    find(:xpath, "//a[@data-action='exit']").click
    sleep 3
    save_screenshot('/tmp/scr1.png', :full => true)
    error = first("div[class='flash flash_error']", visible: false)
    expect(error.text).to eq("Your are not allowed to create sources with siglum 'F-Pn'.")
  end
  
  it "should be warning for incipit higher than 200" do
    set_field("852$x", institution.id)
    set_field("031$a", "211")
    find(:xpath, "//a[@data-action='exit']").click
    sleep 5
    ...
    expect(warning.text).to eq("031$a value '211' is greater than 200")
  end
 
  it "warning should be triggered only one time" do
    set_field("852$x", institution.id)
    set_field("031$a", "211")
    find(:xpath, "//a[@data-action='exit']").click
    save_screenshot('/tmp/scr1.png', :full => true)
  ...
  it "errors should be in a endless loop" do
    set_field("852$x", foreign_institution.id)
    find(:xpath, "//a[@data-action='exit']").click
    sleep 3
    save_screenshot('/tmp/scr1.png', :full => true)
