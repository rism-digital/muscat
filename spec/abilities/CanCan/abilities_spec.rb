...
RSpec.describe User do
  ...
  context 'with person editor abilities' do
    ...
    xit { expect(ability).to be_able_to(:update, person) }
    xit { expect(ability).to be_able_to(:destroy, person) }
    xit { expect(ability).to be_able_to(:create, person) }
  end
end
...
RSpec.describe Role do
  ...
  context "when is a editor" do
    ...
    xit{ is_expected.to be_able_to(:create, Person.new)  }
    xit{ is_expected.to be_able_to(:read, Person.new)  }
    xit{ is_expected.not_to be_able_to(:update, Person.new)  }
    xit{ is_expected.to be_able_to(:update, Person.new(:wf_owner => user.id))  }
    xit{ is_expected.not_to be_able_to(:destroy, Person.new)  }
  end
  ...
  context "when is a admin" do
    ...
    xit{ is_expected.to be_able_to(:create, Person.new)  }
    xit{ is_expected.to be_able_to(:read, Person.new)  }
    xit{ is_expected.to be_able_to(:update, Person.new)  }
    xit{ is_expected.to be_able_to(:destroy, Person.new)  }
  end
end
