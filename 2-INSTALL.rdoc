...
The documentation is updated for Ubuntu 18.14 LTS.
...
Muscat 5 now requires ruby 2.5 by default. 
Rails is installed with the correct version with bundle. 
Ubuntu comes with ruby 2.5 by default so no actions are necessary. 
On older Debian systems the easyest way to get ruby 2.5 is via rbenv adding for all users.
